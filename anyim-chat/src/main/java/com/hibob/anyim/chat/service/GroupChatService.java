...
public class GroupChatService {
    ...
    public ResponseEntity<IMHttpResponse> pullMsg(PullGroupChatMsgReq dto) {
        ...
        if (currentTime - readTime < msgTtlInRedis * 1000) { // 7天内查询Redis
            ...
            if (count > 0) {
                LinkedHashSet<Object> msgIds = (LinkedHashSet)redisTemplate.opsForZSet().rangeByScore(key1, readMsgId + 1, Double.MAX_VALUE, 0, pageSize);
            ...
            return ResultUtil.success(queryMsgFromDB(sessionId, readMsgId, pageSize));
        }
    }
    ...
    public ResponseEntity<IMHttpResponse> history(GroupChatHistoryReq dto) {
        ...
        HashMap<String, Object> resultMap = queryMsgFromDB(sessionId, startTime, endTime, readMsgId, pageSize);
        return ResultUtil.success(resultMap);
    }

    private HashMap<String, Object> queryMsgFromDB(String sessionId, long readMsgId, int pageSize) {
        return queryMsgFromDB(sessionId, null, null, readMsgId, pageSize);
    }

    private HashMap<String, Object> queryMsgFromDB(String sessionId, Date startTime, Date endTime, long readMsgId, int pageSize) {
